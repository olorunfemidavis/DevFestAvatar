...
(function (global, factory) {
  ...
  function createRotationSlider() {
    if (this.properties.rotation_slider) {
      if (!this.options.rotation.slider) {
        this.properties.wrapper.removeChild(
          this.properties.rotation_slider.parentNode
        );
        delete this.properties.rotation_slider;
      } else {
        this.properties.rotation_slider.disabled =
          !this.options.rotation.enable;
      }

      this.properties.rotation_slider.style.width =
        this.properties.container.offsetWidth + "px";
    } else {
      if (this.options.rotation.slider) {
        var sliderContainer = document.createElement("div");
        sliderContainer.classList.add("cropme-rotation-slider");
        var slider = (this.properties.rotation_slider =
  ...
  function createSlider() {
    function changeSliderParameter() {
      var diff =
        (this.properties.wrapper.offsetWidth -
          this.properties.container.offsetWidth) /
        2;
    ...
    if (this.properties.slider) {
      ...
      if (this.options.zoom.slider) {
        var self = this;
        var sliderContainer = (this.properties.sliderContainer =
        ...
        sliderContainer.style.width =
          this.properties.container.offsetHeight + "px";
        sliderContainer.appendChild(slider);
        this.properties.wrapper.insertBefore(
          sliderContainer,
          this.properties.wrapper.firstChild
        );
  ...
  function createViewport() {
    ...
    if (options.viewport.border.enable) {
      var viewport_border =
        (container.offsetWidth - options.viewport.width) / 2;
    ...
    this.properties.viewport.style.borderWidth =
      options.viewport.border.width + "px";
  ...
  function createContext() {
    ...
    var move = function move(e) {
      ...
      if (e.touches && e.touches.length > 1) {
        ...
        if (self.options.zoom.enable) {
          ...
          self.properties.scale = self.properties.slider.value =
            touches_dist / self.properties.od;
        }
      } else {
    ...
    var mousewheel = function mousewheel(e) {
      e.preventDefault();
      var scale =
        self.properties.scale + (e.wheelDelta / 1200) * self.properties.scale;
  ...
  function createCanvas(options) {
    ...
    var x =
      xs * (imageData.x - viewportData.x - this.options.viewport.border.width);
    var y =
      ys * (imageData.y - viewportData.y - this.options.viewport.border.width);
  ...
  var Cropme =
    ...
    (function () {
      function Cropme(el, options) {
        ...
        this.properties.wrapper.className +=
          " cropme-wrapper " + this.options.customClass;
      ...
      _createClass(Cropme, [
          ...
          value: function resize() {
            ...
            if (container.offsetWidth > this.options.viewport.width) {
              ...
              container.style.width =
                ...
                this.options.viewport.border.width * 2 +
                "px";
            }
          },
        },
              ...
              _this.properties.image.onload = function () {
                ...
                if (_typeof(obj.position) === "object") {
                  ...
                  options.transformOrigin =
                    obj.position.origin === "object" ? "viewport" : "image";
                }
        ...
        },
        {
          ...
          value: function rotate(deg) {
            this.properties.deg = deg;
            var origin =
              this.properties.image.style.transformOrigin.split("px ");
        ...
        },
        {
          ...
          value: function position() {
            var position = {
              x: parseFloat(
                (this.properties.x - this.properties.ox).toFixed(3)
              ),
              y: parseFloat(
                (this.properties.y - this.properties.oy).toFixed(3)
              ),
        ...
        },
        {
          ...
          value: function destroy() {
            ...
            this.properties.wrapper.className =
              this.properties.wrapper.className.replace("cropme-wrapper", "");
            delete this.options;
  ...
  var containerLenght = 500;

  if (window.screen.height < 800)
    //Small Size Tablets or PC.  15" Notebook
    containerLenght = 320;

  if (screen.width < 600) containerLenght = 340;

  if (screen.width < 380) containerLenght = 280;

  if (screen.width < 320) containerLenght = 220;
